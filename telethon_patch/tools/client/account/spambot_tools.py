...
from telethon_patch.attachments.memory.types import Types as memory_types
...
class AccountSpambotTools:
    ...
    async def load_spamblock_info(
        ...
        async with self.conversation(self._spambot_id) as conv:
            for _ in range(2):
                try:
                    ...
                    await self(tl_functions.contacts.UnblockRequest(id=self._spambot_id))
        ...
        message_has_url = any([isinstance(_, tl_types.MessageEntityTextUrl) for _ in message.entities or []])
