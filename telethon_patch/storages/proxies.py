...
from telethon_patch.lib.metaclasses.singleton import _SingletonMeta
...
class Proxy:
    ...
    def clear_errors(self):
        ...
        self.__last_errors = [_ for _ in self.__last_errors if _ > time.time() - _LIMIT_PROXY_ERRORS_TIME]
...
class ProxyStorage(metaclass=_SingletonMeta):
    ...
    def add_proxy(self, proxy: Union[str, Proxy], network_type: Literal["socks5", "http"]):
    ...
    def get_proxy(self, random_choice: bool = False, usage_index_choice: bool = True) -> Optional[Proxy]:
        ...
        if all([random_choice, usage_index_choice]):
            raise ValueError("random_choice and usage_index_choice cannot be True at the same time")
        ...
        if usage_index_choice:
            proxy = next(iter(sorted(available_proxies, key=lambda x: x.last_used, reverse=True)))
